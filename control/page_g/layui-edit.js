    ...
    var active = {
        seting: function(othis){
            ...
            layer.open({
                ...
                ,yes: function(index){ layer.close(index);window.location.reload();} // yes
                ,cancel: function(){window.location.reload();} // cancel
            });
        }
        ,confirm_del:function(othis){
            var url = othis.attr("href"),name = othis.data('name'),key_id = othis.data('key'),that = this;
            layer.confirm('提示:删除将无法恢复!你确认要删除吗?' ,{icon: 2, title:'提示:'+name}
                , function(index){layer.close(index); window.location.href = url;} // yes
